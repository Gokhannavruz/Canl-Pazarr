...
class _PaywallState extends State<Paywall> with TickerProviderStateMixin {
  ...
  Future<void> _initializePackages() async {
    try {
      ...
      // Select a default package (weekly)
  ...
  void _selectDefaultPackage() {
    // Select the weekly package by default
    for (int i = 0; i < _sortedPackages.length; i++) {
      if (_sortedPackages[i].packageType == PackageType.weekly) {
        _selectedPackageIndex = i;
        return;
      }
    }

    // If no weekly package, select the first package
  ...
  String _getTrialText(Package package) {
    return ''; // No trial text displayed
  }
