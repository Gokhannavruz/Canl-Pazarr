...
class PaywallFirstLaunch extends StatefulWidget {
  final Offering offering;

  const PaywallFirstLaunch({Key? key, required this.offering})
      : super(key: key);

  @override
  _PaywallFirstLaunchState createState() => _PaywallFirstLaunchState();
}

class _PaywallFirstLaunchState extends State<PaywallFirstLaunch> {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: IconButton(
          ...
          onPressed: () => // DiscoverPage'e yönlendir
  ...
  void _subscribeNow() async {
    try {
      ...
      if (entitlement?.isActive ?? false) {
        // Satın alma başarılı oldu, kullanıcı durumunu güncelle
        await updatePremiumStatus(true);
        await updateCredits(9999999);

        // CountryStateCityForFirstSelect sayfasına yönlendir
        if (context.mounted) {
          Navigator.of(context).pushReplacement(
            MaterialPageRoute(
              builder: (context) => const CountryStateCityForFirstSelect(),
            ),
          );
        }
      } else {
